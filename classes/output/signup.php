...
$PAGE->set_url('/theme/easylaws/classes/output/signup.php');
...
if (isset($_POST['email'])){
    ...
    if ($DB->get_record('user', array('email' => $user->email, 'deleted' => 0))) {
        notice ('You already have an account. Please login instead.', new moodle_url('/login'));
    }

    $user->username = trim(strtolower($user->email));
    // print_object($_POST);
